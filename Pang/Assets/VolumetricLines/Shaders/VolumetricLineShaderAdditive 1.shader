...
Shader "VolumetricLine/VolumetricLineAdditive" {
	...
	_Color("Main Color", Color) = (1,1,1,1)
		...
		_GlowFactor("Glow Factor", Range(0.0, 1.0)) = 0.9
	}
		SubShader{
...
#include "UnityCG.cginc"
	...
	v2f vert(a2v v)
	{
		...
		float scaledLineWidth = lerp(0.1f, 0.3f, abs(sin(_Time.y)));
#if FOV_SCALING_ON
	...
	float4 frag(v2f i) : COLOR
	{
		...
		return IF(tx.a > lerp(0.5f, 0.95, abs(sin(_Time.y))), float4(1.0, 1.0, 1.0, tx.a), tx * _Color)
	}
		ENDCG
	}
	}
